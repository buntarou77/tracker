...
import { getCurrencySymbol } from '../lib/symbols';
...
const BankAccount = () => {
  const t = useTranslations('bankAccount'); // UI translations
  // If you had a separate errors namespace, you could use it, but we'll keep all in one for simplicity.
    ...
    if (bankNames.length >= 6) {
      ...
      try {
        if (!newAccount.name || !newAccount.currency || !newAccount.balance) {
          alert(t('pleaseFillAllFields')) // <-- translated
        } else {
  ...
  const deleteAccount = async (accountId: string, accountName: string, e: React.MouseEvent<HTMLButtonElement>) => {
    e.stopPropagation();
    if (window.confirm(t('deleteConfirm', { name: accountName }))) { // <-- translated confirm
      try {
        ...
        if (response.ok) {
          ...
          alert(`${t('errorTitle')}: ${errorData.error || t('errorDeletingAccount')}`); // <-- translated
        }
      } catch (error) {
        alert(t('errorDeletingAccount')); // <-- translated
      }
    }
  };
        ...
        <div className='flex items-center justify-start'>
          ...
          <div className="flex flex-col items-start justify-center gap-0.5">
            <div className='flex gap-1 items-center'>
              ...
              <span className="font-bold text-[75%] text-white/90 leading-tight">{activeBank.name || t('noName')}</span>
            </div>
          ...
          >
            ...
            <ul className="mb-2">
              {bankNames.length > 0 ? (
                bankNames.map((account, index) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      title={t('editAccountTitle')} // <-- translated
                    >
                    ...
                    <button
                      ...
                      title={t('deleteAccountTitle')} // <-- translated
                    >
                ...
                <li className="py-1 text-gray-400 text-sm pl-2">{t('noAccounts')}</li>
              )}
            </ul>
            {addBankAccountForm ? (
              <div className="p-2 border-t border-gray-700 w-[200px] max-w-xs mx-auto">
                <form className="space-y-2 w-full" onSubmit={(e) => addBankAccount(e)}>
                  <input
                    type="text"
                    placeholder={t('accountNamePlaceholder')} // <-- translated
                  ...
                  />
                  <textarea
                    placeholder={t('notesPlaceholder')} // <-- translated
                    className="w-full px-2 py-1 text-sm bg-gray-700 border border-gray-600 rounded focus:ring-1 focus:ring-green-500 focus:outline-none"
                    rows={2}
                  ...
                  <div className="flex gap-1">
                    <input
                      type="number"
                      placeholder={t('amountPlaceholder')} // <-- translated
              ...
              <button
                ...
                className="w-full p-2 text-sm font-medium text-center text-white bg-gray-700 hover:bg-gray-600 transition-colors border-t border-gray-700 rounded-lg"
              >
                {t('addAccountButton')} {/* <-- translated */}
              </button>
            )}
            <button
            ...
            >
              ...
              <img className='h-4 w-4' src={settingSvg.src} alt="settings" />
              </div>
              {t('settingsButton')} 
            </button>
          ...
          <div className="w-80 bg-slate-800/95 backdrop-blur-sm rounded-lg shadow-2xl border border-slate-600/50 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-slate-700/50 to-slate-600/50">
              <div className="flex items-center">
                ...
                <h3 className="font-medium text-slate-200 text-sm">{t('notificationTitle')}</h3>
              </div>
              <button
            ...
            <div className="px-4 py-3">
              <p className="text-sm text-slate-300">{t('tooManyAccounts')}</p>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="w-80 bg-slate-800/95 backdrop-blur-sm rounded-lg shadow-2xl border border-slate-600/50 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-slate-700/50 to-slate-600/50">
              <div className="flex items-center">
                ...
                <h3 className="font-medium text-slate-200 text-sm">{t('errorTitle')}</h3>
              </div>
              <button
            ...
            <div className="px-4 py-3">
              <p className="text-sm text-slate-300">{t('errorRegistering')}</p>
            </div>
          </div>
        </div>
      )}
