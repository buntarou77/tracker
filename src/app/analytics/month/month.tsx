...
import { preparePieTransactions, prepareMonthBarData } from '@/app/utils/createData';
import { prepareLineData, preparePieData, prepareDoughnutData, prepareBarData } from '@/app/utils/prepareData';
...
export default function MonthAnalytics() {
  ...
  const [selectedDate, setSelectedDate] = useState<MonthAnalyticsProps>({ year: '', month: '' });
  ...
  useEffect(() => {
    if (selectedDate.year && selectedDate.month && trans.length > 0) {
      const startDate = new Date(`${selectedDate.year}-${selectedDate.month}-01`);
  ...
  const lossBarLabels = prepareMonthBarData(lossTrans).label;
  const lossBarData = prepareMonthBarData(lossTrans).data;
  const barLossData = prepareBarData(lossBarData, lossBarLabels);
  const gainBarLabels = prepareMonthBarData(gainTrans).label;
  const gainBarData = prepareMonthBarData(gainTrans).data;
    ...
    return (
      ...
      >
        <div className="flex flex-col">
          <label htmlFor="month" className="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-1">
            Month
          </label>
          <select
            id="month"
    ...
    <div className="header bg-dark m-auto flex justify-center flex-col pl-[100px] pr-[100px]">
      <div>
        ...
        <p className={`${endBudget > startBudget ? 'text-[#bdffab]' : 'text-[red]'} opacity-[0.8]`}>
          <span className='text-[white]'>Monthly Result:</span> {`${monthRes}$`}
        </p>
      </div>
      ...
      <div className='justify-center flex flex-col items-left pl-[300px] pr-[300px]'>
        <div className='border-[2px] border-[#5e5e5e] rounded-[10px]'>
          <p className='text-[20px] font-[800]'>Top 5 categories this month:</p>
