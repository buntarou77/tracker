...
export function useBankAccounts() {
  ...
  const getBalance = async (bankName: string) => {
    if (!state.user.isAuthenticated || !state.user.data) return null;

    try {
      const response = await fetch(
        `/api/getBalanceRedis?login=${state.user.data.login}&bankName=${bankName}`,
        { credentials: 'include' }
      );
...
export function useTransactions() {
  ...
  const getTransactionsByType = (type: 'income' | 'expense') => {
    return state.transactions.data.filter(trans => trans.type === type);
  };

  const getTransactionsByDateRange = (startDate: string, endDate: string) => {
    return state.transactions.data.filter(trans => {
...
export function usePlans() {
  ...
  const updatePlan = async (planData: Plan) => {
    ...
    try {
      const response = await fetch('/api/rewritePlan', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        return { success: false, error: errorData.error || 'Не удалось обновить план' };
      }
    } catch (error) {
...
export function useStatistics() {
  ...
  const getYearlyStatistics = (year: number) => {
    const yearTransactions = state.transactions.data.filter(trans => {
      const transDate = new Date(trans.date);
